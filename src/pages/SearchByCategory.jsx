...
import VendorsByCategory from "../components/LandingPage/VendorsByCategory.jsx";
  ...
  useEffect(() => {
    const fetchByCategory = async (categoryId) => {
      const userRes = await axios.get(
        `${API_URL}/api/vendors-by-category?category=${categoryId}`
      ); //Fix this and change to local government
      if (userRes.status == 200 || userRes.status == 201) {
        setListings(userRes.data);

        localStorage.setItem(
          `vendorsCategory${categoryId}`,
        ...
        <header className="h-screen p-4 md:h-[644px] w-full bg-header_image bg-cover m-0 top-0 flex justify-between py-8 items-center flex-col">
          ...
          <div className="flex-1 flex flex-col justify-center p-4 gap-8 items-center">
            ...
            <p className="text-[#F2F2F2] text-[16px] md:text-[24px] font-normal text-center w-[80vw]">
              {user.lang == "ha"
                ? "JMarket ke kawo abubuwa mafi kyau daga Plateau zuwa garaku. Fara tafiyanka yanzu"
                : `JMarket Puts Plateau’s Best at Your Fingertips. Your Story of Connection Begins Now!`}
            </p>
