...
const {categoryId} = useParams()
    ...
    useEffect(()=>{
        ...
        const fetchByCategory= async (categoryId)=> {
             ...
             const userRes = await axios.get(`${API_URL}/api/vendors-by-category?category=${categoryId}`) //Fix this and change to local government
             if(userRes.status == 200 || userRes.status == 201)
             {
                setListings(userRes.data)

             localStorage.setItem(`vendorsCategory${categoryId}`, JSON.stringify(userRes.data))
        ...
        <>
            <main className='min-h-screen'>
                ...
                <div className="flex flex-col md:flex-row px-2 md:px-10 py-4">
                    ...
                    <VendorsByCategory foundItems={listings} title={"Top Category Vendors"} />
