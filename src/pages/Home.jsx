...
import {
  getAllVendorsCategoryService,
  getVendorByGpsService,
} from "../service/vendorListingService.js";
      ...
      <main className="min-h-screen">
        <header className="h-screen p-4 md:h-[644px] w-full bg-header_image bg-cover m-0 top-0 flex justify-between py-8 items-center flex-col">
          ...
          <div className="flex-1 flex flex-col justify-center p-4 gap-8 items-center">
            <h1 className=" text-white font-semibold text-[26px] w-[80vw] text-center md:text-[48px] ">
              {user.lang == "ha"
                ? "Sami diyawa, haduwa mafi kyau"
                : `Discover more, connect better`}{" "}
            </h1>

            <p className="text-[#F2F2F2] text-[16px] md:text-[24px] font-normal text-center w-[80vw]">
              {user.lang == "ha"
                ? "JMarket ke kawo abubuwa mafi kyau daga  zuwa garaku. Fara tafiyanka yanzu"
                : `JMarket Puts Pateau’s Best at Your Fingertips. Your Story of Connection Begins Now!`}
            </p>
            ...
            <div className="flex w-[90vw] md:w-auto flex-col md:flex-row gap-4">
              <div className="h-[48px] w-full overflow-hidden md:w-[674px] bg-white border items-center flex rounded-full pl-3 pr-3">
                <img
                  src={searchIcon}
                  className=" h-[24px] w-[24px] flex-initial"
                />

                <div className="flex-1 md:flex-1">
                  <input
                    className="pl-4 appearance-none outline-none"
                    placeholder={
                      user.lang == "ha"
                        ? "Me kake nema?"
                        : `What are you looking for?`
                    }
                    onChange={(elem) => handleSetSearchKey(elem.target.value)}
                    value={searchKey}
                  />
                </div>
                <img
                ...
                <div className="flex-1 md:flex-2 flex">
                  <select
                    className="flex-1 md:flex-1 text-[#9caccb] outline-none appearance-none"
                    placeholder="Location"
                  >
                    <option value="">
                      {" "}
                      {user.lang == "ha" ? "Ina?" : `Location`}
                    </option>
                    {localGovernmentAreas.map((lga) => {
                      return (
                        <option key={lga.slug} value={lga.slug}>
                          {lga.name}
                        </option>
                      );
                    })}
              ...
              <button
                ...
                className="bg-[#993420] h-[2rem] md:h-auto text-white rounded-3xl pl-4 pr-4 text-[16px] font-semibold"
              >
            ...
            <button
              onClick={handleVendorsNearMe}
              className="bg-[#993420] fixed right-0 bottom-0 me-[30px] mb-[30px] h-[2rem] md:h-auto text-white rounded pl-4 pr-4 p-2 text-[16px] font-semibold"
            >
            ...
            <div className="flex gap-4 md:gap-8">
              <div className="w-[96px] h-[98px] flex flex-col items-center justify-start">
                ...
                <p className="text-[#808080] text-[14px] md:text-[16px] font-semibold">
                  {" "}
                  {user.lang == "ha" ? "Masu Aiki" : `Top Vendors`}
                </p>
              </div>
