...
const WithdrawalRequestModal = ({ handleModal, fetchedUser }) => {
  ...
  const handleAddListing = async () => {
    if (location.latitude == null || location.longitude == null) {
      return alert(
        "Can't submit. Please ensure you have granted all required device access."
      );
    }
    ...
    if (!fetchedUser.id) {
      ...
      alert(
        "We could not identify you. Kindly reload the page or login and try again"
      );
      return true;
    ...
    <div className="fixed h-screen w-full bg-[#808080a3] left-0 right-0 bottom-0 flex flex-col items-center justify-center p-4 overflow-hidden">
      <div className="bg-white p-4 rounded-lg shadow-2xl min-h-[400px] min-w-[70%] md:min-w-[70%] flex flex-col gap-2 overflow-y-scroll">
        ...
        <div className="">
          <p>Bank</p>
          <input
            onChange={(elem) => setBank(elem.target.value)}
            value={bank}
            className="border px-4 py-2 w-full"
        ...
        <div className="flex-1 flex items-center gap-2">
          <input
            onChange={(elem) => setTerms(elem.target.checked)}
            className="border"
            type="checkbox"
          />
          <p>
            By proceeding, you agree to{" "}
            <Link to="/tc">JMarket's withdrawal's terms and conditions</Link>
          </p>
        </div>
