...
const AddServiceModal = ({ handleModal, fetchedUser }) => {
  ...
  const handleAddListing = async () => {
    if (location.latitude == null || location.longitude == null) {
      return alert(
        "Can't submit. Please ensure your have granted all required device access."
      );
    }
    ...
    if (
      fetchedUser.id == "" ||
      fetchedUser.id == undefined ||
      fetchedUser.id == null
    ) {
      console.log("You are probably not logged in");
      alert(
        "We could not identify you. Kindly reload page or login and try again"
      );
      return true;
    } else if (
      localGovernment == "" ||
    ...
    if (urls.length > 0) {
      ...
      if (res.status === 200 || res.status === 201) {
        ...
        for (const [index, item] of listingPhotos.entries()) {
          await axios
            .post(`${API_URL}/api/listing-images`, {
              name: `Photo ${index + 1}`,
              serviceID: serviceID,
              image_url: item,
              title:
              ...
              description:
                index == 0
                  ...
                  ? photoTwoDescription
                  : index == 2
                  ? photoThreeDescription
                  : index == 3
                  ? photoFourDescription
                  : "No Description",
              price:
                index == 0
                  ? photoOnePrice
        ...
        alert(
          "Service submitted successfully! Please allow up to 24 hours for your service to be approved."
        );
        handleModal();
      } else {
    ...
    <div className="fixed h-screen w-full bg-[#808080a3] left-0 right-0 bottom-0 flex flex-col items-center justify-center p-4 overflow-hidden">
      <div className="bg-white p-4 rounded-lg shadow-2xl min-h-[400px] min-w-[70%] md:min-w-[70%] flex flex-col gap-2 overflow-y-scroll">
        <div className="flex justify-between">
          ...
          <div
            onClick={handleModal}
            className="size-[50px] bg-[red] flex justify-center items-center rounded text-white font-bold"
          >
            X
          </div>
        </div>
        ...
        <div className="">
          ...
          <textarea
            ...
            value={description}
            className="border px-4 py-2 w-full h-[40vh]"
            name="email"
        ...
        <div className="flex justify-between gap-4">
          ...
          <div className="flex-1">
            ...
            <select
              ...
              onChange={(elem) => setLocalGovernment(elem.target.value)}
            >
              <option value="">...select LGA</option>
              <option value="1">Jos North</option>
              <option value="2">Jos South</option>
              <option value=""></option>
            </select>
          </div>
          ...
          <div className="flex-1">
            <p>C.A.C (Optional)</p>
            <input
              onChange={(elem) => setCAC(elem.target.value)}
              value={cac}
              className="border px-4 py-2 w-full"
        ...
        <div className="flex-1">
          <p>Cover Photo</p>
          <input
            onChange={handleFileChange(setCoverPhoto, "coverPhoto")}
            className="border px-4 py-2 w-full"
            name="coverPhoto"
            type="file"
          />
        ...
        <div className="flex-1">
          ...
          <input
            onChange={handleFileChange(setPhotoOne, "photoOne")}
            className="border px-4 py-2 w-full"
            name="email"
            placeholder="Enter CAC"
            type="file"
          />
          <input
            onChange={(e) => setPhotoOneTitle(e.target.value)}
            value={photoOneTitle}
            className="border px-4 py-2 w-full"
            placeholder="Title for Photo 1"
          />
          <textarea
            onChange={(e) => setPhotoOneDescription(e.target.value)}
            value={photoOneDescription}
            className="border px-4 py-2 w-full"
            placeholder="Description for Photo 1"
          />
          <input
            onChange={(e) => setPhotoOnePrice(e.target.value)}
            value={photoOnePrice}
            className="border px-4 py-2 w-full"
            placeholder="Price for Photo 1"
          />
        </div>
        ...
        <div className="flex-1">
          ...
          <input
            onChange={handleFileChange(setPhotoTwo, "photoTwo")}
            className="border px-4 py-2 w-full"
            name="email"
            placeholder="Enter CAC"
            type="file"
          />
          <input
            onChange={(e) => setPhotoTwoTitle(e.target.value)}
            value={photoTwoTitle}
            className="border px-4 py-2 w-full"
            placeholder="Title for Photo 2"
          />
          <textarea
            onChange={(e) => setPhotoTwoDescription(e.target.value)}
            value={photoTwoDescription}
            className="border px-4 py-2 w-full"
            placeholder="Description for Photo 2"
          />
          <input
            onChange={(e) => setPhotoTwoPrice(e.target.value)}
            value={photoTwoPrice}
            className="border px-4 py-2 w-full"
            placeholder="Price for Photo 2"
          />
        </div>
        ...
        <div className="flex-1">
          ...
          <input
            onChange={handleFileChange(setPhotoThree, "photoThree")}
            className="border px-4 py-2 w-full"
            name="email"
            placeholder="Enter CAC"
            type="file"
          />
          <input
            onChange={(e) => setPhotoThreeTitle(e.target.value)}
            value={photoThreeTitle}
            className="border px-4 py-2 w-full"
            placeholder="Title for Photo 3"
          />
          <textarea
            onChange={(e) => setPhotoThreeDescription(e.target.value)}
            value={photoThreeDescription}
            className="border px-4 py-2 w-full"
            placeholder="Description for Photo 3"
          />
          <input
            onChange={(e) => setPhotoThreePrice(e.target.value)}
            value={photoThreePrice}
            className="border px-4 py-2 w-full"
            placeholder="Price for Photo 3"
          />
        </div>
        ...
        <div className="flex-1">
          ...
          <input
            onChange={handleFileChange(setPhotoFour, "photoFour")}
            className="border px-4 py-2 w-full"
            name="email"
            placeholder="Enter CAC"
            type="file"
          />
          <input
            onChange={(e) => setPhotoFourTitle(e.target.value)}
            value={photoFourTitle}
            className="border px-4 py-2 w-full"
            placeholder="Title for Photo 4"
          />
          <textarea
            onChange={(e) => setPhotoFourDescription(e.target.value)}
            value={photoFourDescription}
            className="border px-4 py-2 w-full"
            placeholder="Description for Photo 4"
          />
          <input
            onChange={(e) => setPhotoFourPrice(e.target.value)}
            value={photoFourPrice}
            className="border px-4 py-2 w-full"
            placeholder="Price for Photo 4"
          />
        </div>

        <div className="flex-1 flex items-center gap-2">
          <input
            onChange={(elem) => setTerms(elem.target.value)}
            value={terms}
            className="border"
            type="checkbox"
          />
          <label>
            ...
            <Link to={"/tc"}>JMarket vendors terms and conditions </Link>
          </label>
        </div>
        ...
        <div className="flex">
          <button
            ...
            className="bg-[#993420] w-full text-white p-4 rounded-lg flex justify-center items-center"
          >
            <span>{inProgress == true ? "Proceeding..." : "Add Service"}</span>
