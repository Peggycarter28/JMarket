...
const CartPaymentModal = ({ handleModal, fetchedUser, cartItems, total_amount=null }) => {
    ...
    const handlePayment = async () => {
        if (stage == 0) {
            ...
            const data = {
                ...
                "amount": (parseInt(total_amount) + (5 / 100 * total_amount)) * 100,
                "service_id": 0,
    ...
    const handleWindowClose = async () => {
        ...
        if (verify.status == 200 || verify.status == 201) {
            ...
            const transaction_data = {
                reference: verify.data.reference+'_'+index,
    ...
    return (
        <div className="fixed z-100 h-screen w-full bg-[#808080a3] left-0 right-0 bottom-0 flex flex-col items-center justify-center">
            <div className="bg-white p-4 rounded-lg shadow-2xl min-h-[400px] min-w-[300px] md:min-w-[400px] flex flex-col gap-2">
                ...
                <div className="flex justify-between">
                    <p>Service fee</p> <p>NGN {total_amount == null ? 
                        <input onChange={((elem)=>setAmount(elem.target.value))} className='border' type="number" />
                ...
                <div className="flex justify-between">
                    <p>Charge (5%)</p> <p>NGN {(amount * 5 / 100).toLocaleString()}</p>
                </div>

                <div className="flex justify-between">
                    <p>Items in cart: </p> <p> {cartItems.length}</p>
                </div>

                <div className="flex justify-between border-t pt-2 mt-4 text-[1.5rem] font-semibold">
                    <p className="">Total</p> <p>NGN{(parseInt(amount) + (5 / 100 * amount)).toLocaleString()}</p>
                </div>
                ...
                <div className="flex">
                    <button onClick={handlePayment} className="bg-[#ef6c00] w-full text-white p-4 rounded-lg flex gap-2 justify-center items-center">{stage == 0 ? "Continue" : stage == 1 ? "Proceed to Paystack" : stage == 2 ? "Checking Payment status" : ""}
