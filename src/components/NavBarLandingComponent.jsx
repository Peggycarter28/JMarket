import siteLogo from "/JMarket.png";
  ...
  return (
    <div className="flex w-full">
      <div className="flex w-full justify-end absolute top-0 right-0 mt-3 mr-3 index-[45]">
        <select
          onChange={(elem) => handleLangUpdate(elem.target.value)}
      ...
      <nav className="flex w-full justify-between md:justify-around items-center p-2 md:p-8">
        ...
        <div className="flex gap-1">
          {user.isLoggedIn == true && user.username !== null && (
            <p className="mr-2 mt-2 text-[#d9d9d9] font-semibold text-[10px] md:text-[14px]">
              <span className="text-xl">Hello, </span> {`${user.username}`}.
              <br />
              Go to{" "}
              <Link
                className="border-b-2 border-orange-300 hover:border-orange-500 transition-all duration-300"
                to={"/dashboard/user/"}
              >
                Dashboard
              </Link>
      ...
      {/* Mobile Nav Menu */}
      {showMenu == true && (
        <div className="absolute bg-[white] w-full h-[70vh] shadow-lg left-0 top-0 flex flex-col">
